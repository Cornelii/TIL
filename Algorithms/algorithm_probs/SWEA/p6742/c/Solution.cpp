	...
	for(test_case = 1; test_case <= T; ++test_case)
	{
		...
		for(int jmi=0;jmi<N-2;jmi++){
			for(int jmii=jmi+1;jmii<N-1;jmii++){
				...
				angle=atan((double)vec1[1]/vec1[0]);
				ssA=sin(angle);
				cA=cos(angle);
				p2_x=cA*vec1[0]+ssA*vec1[1];

                for(int jmiii=jmii+1;jmiii<N;jmiii++){
                    vec2[0]=points[jmiii][0]-points[jmi][0];
                    vec2[1]=points[jmiii][1]-points[jmi][1];
                    p3_y=cA*vec2[1]-ssA*vec2[0];

                    area=abs(p2_x*p3_y);
                    if(area>A || abs(A-area)<=1e-6){
                        if(B>area || abs(B-area)<=1e-6){
                            count+=1;
                        }
                    }
                }
