...
def line_processor(word,velocity0,distance0):
    command=list(map(int,word.split()))

    if command[0]==0:
        return velocity0,distance0+velocity0
    elif command[0]==1:
        return velocity0+command[1],distance0+velocity0+command[1]
    else:# command==2
        if not velocity0<=command[1]:
            return velocity0-command[1], distance0+velocity0-command[1]
        else:
            return 0,distance0
...
for case in range(1,T+1):
    ...
    for _ in range(num):
        line=input()
        velocity,distance=line_processor(line,velocity,distance)
