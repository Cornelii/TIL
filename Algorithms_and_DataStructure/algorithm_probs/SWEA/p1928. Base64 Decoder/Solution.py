...
def line_processor(words):
    word_length=len(words)
    binary_collector=["{:06b}".format(base64.find(word)) for word in words]
    binary="".join(binary_collector)
    result=[]
    for idx in range(int(word_length*3/4)):
        _ascii=binary[idx*8:idx*8+8]
        result.append(chr(int(_ascii,2)))
    return "".join(result)
